...
The core business logic lives in ``app.services.graph_query.run_graph_query()``.
This module only provides two adapter functions:

  - ``build_graph_query_tool()`` — returns a LangChain ``@tool`` for LangGraph.
  - ``register_mcp_tool()``      — registers the tool on a ``FastMCP`` server.
"""
...
from app.services.graph_query import run_graph_query
...
# Module-level placeholder — replaced by `build_graph_query_tool()` in lifespan.
