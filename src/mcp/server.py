...
`src/main.py` via `app.mount(settings.mcp_path, mcp.http_app(path="/"))`.
...
def register_all_tools(settings: Settings) -> None:
    ...
    """
    from src.graph.connection import get_graph  # noqa: F401 (import triggers validation)
    from src.llm.factory import get_llm_from_settings
    from src.mcp.tools import graph_query as gq_module
    from src.mcp.tools import schema_info as si_module
    from src.mcp.tools import vector_search as vs_module
    from src.core.dependencies import get_schema_cache_instance
